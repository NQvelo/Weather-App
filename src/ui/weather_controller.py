...
class WeatherController:
    ...
    def _process_weather_data(self, data: Dict, city: str) -> Dict:
        ...
        Group 1's data format:
        {
            ...
            'pressure': 1013,
            'humidity': 65,
            'sunrise': 1234567890,  # Unix timestamp
            'sunset': 1234567890,   # Unix timestamp
            'visibility': 10.0,     # in km
            'uv_index': 'Moderate (4-6)',
            'timezone': 3600,        # Timezone offset in seconds
        ...
        """
        from api.weather_api import format_sunrise_sunset
        ...
        processed = {
            ...
            # Sunrise/Sunset/Visibility/UV Index
            'sunrise': format_sunrise_sunset(
                data.get('sunrise'),
                data.get('timezone', 0)
            ) if data.get('sunrise') else "N/A",
            'sunset': format_sunrise_sunset(
                data.get('sunset'),
                data.get('timezone', 0)
            ) if data.get('sunset') else "N/A",
